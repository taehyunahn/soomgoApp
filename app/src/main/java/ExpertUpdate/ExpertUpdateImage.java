...
import AccountUpdate.AccountUpdateImage;

public class ExpertUpdateImage extends AppCompatActivity {


    ImageView iv_userPhoto;
    SharedPreferences sharedPreferences;
    SharedPreferences.Editor editor;
    String userSeq, userProfileImage, expertProfileImage, userEmail, userProfileImageIntent, expertProfileImageIntent;
    Uri selectedUri;
    Bitmap bitmap;
    String encodeImage;
    Button button;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_expert_update_image);
...
//        profileImageShow(userProfileImageIntent, iv_userPhoto);
        ...
        expertProfileImageIntent = intent.getStringExtra("expertProfileImageIntent");
        profileImageShow(expertProfileImageIntent, iv_userPhoto);
...
//        }
        ...
        Log.i(TAG, "expertProfileImageIntent = " + expertProfileImageIntent);
        if(expertProfileImageIntent != null && !expertProfileImageIntent.equals("0")){
            Glide.with(getApplicationContext()).load(expertProfileImageIntent).circleCrop().into(iv_userPhoto);
        }
        ...
        iv_userPhoto.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ImagePicker.with(ExpertUpdateImage.this)
    ...
    private void imageStore(Bitmap bitmap) {
        ...
        String url = serverAddress + "updateProfileImageExpert.php";
...
//                userProfileImage = response;
    ...
    private void profileImageShow(String expertProfileImage, ImageView iv_userPhoto) {
        // ----- 프로필 설정 (case 1~3)
        // case 1. 카카오아이디 최초 로그인 경우, https: 포함 여부 -> glide 사용 (단순 사용)
        if (expertProfileImage.contains("https:")) {
            Log.e("AccountUpdateActivity", "프로필 설정 case 1 (https:)");
            Glide.with(getApplicationContext()).load(expertProfileImage).into(iv_userPhoto);

            // case 2. 앱 내부에서 사진 변경하여 서버에 jpeg 형식으로 저장한 경우, https: 포함 여부 -> glide 사용 (서버 IP 주소 포함)
        } else if (expertProfileImage.contains("jpeg")) {
            Log.e("AccountUpdateActivity", "프로필 설정 case 2 (jpeg)");
            Glide.with(getApplicationContext()).load(serverAddress + "image/" + expertProfileImage).into(iv_userPhoto);

            // case 3. 이메일 로그인 했으며, 최초 등록한 이미지가 없는 경우(sharedPreference null), 아무것도 작동하지 않도록 함 - 기본 이미지
        } else if (expertProfileImage.equals("1")) {
